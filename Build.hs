...
otherYears = S.fromList [2016 .. 2023]
...
main = shakeArgs opts $ do
  ...
  "README.md" %> \fp -> do
    ...
    let mkRow d = case M.lookup d days of
          Just True ->
            printf
              "| Day %2d    | [x][d%02dr]   | [x][d%02dg] | [x][d%02dh]  | [x][d%02db]  |"
              d
              d
              d
              d
          ...
          Just False ->
            printf
              "| Day %2d    |             | [x][d%02dg] | [x][d%02dh]  | [x][d%02db]  |"
              d
              d
              d
              d
  ...
  "_reflections/*.xml" %> \fp -> do
    ...
    Stdout html <-
      cmd
        ("pandoc -t html -f markdown" :: String)
        (Stdin refl)
    let time =
          ZonedTime
            ( LocalTime
                (fromGregorian year 12 d)
                (TimeOfDay 1 0 0)
            )
            (read "EST")
        ctx =
