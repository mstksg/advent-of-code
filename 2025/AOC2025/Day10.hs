...
day10b =
  MkSol
    ...
    , sShow = show
    , sSolve = fmap sum . traverse (traceShowId . go)
    -- noFail $ sum . map go
    }
  where
...
backsolve' maxSearch augMat pivots = mapMaybe (execStateT (traverse go (reverse finites) >> validate)) starting
  where
