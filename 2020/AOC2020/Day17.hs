...
genVecRunIxPascal n mx x = go x (mx, n)
  where
    go :: Int -> (Int, Int) -> NonEmpty Int
    go q (!m, !k) = case chompPascal q (m, k) of
      (j, _, (0, k')) -> j :| [k']
      (j, r, (m', k')) -> j NE.<| go r (m', k')
...
stepper ::
  -- | how big the xy plane is
  Int ->
  -- | neighbor getter function (please cache)
  (Int -> IntMap LiveCount) ->
  -- | alive set: map of <x.y> to all zw+ points (pascal coords)
  IntMap IntSet ->
  IntMap IntSet
    ...
    prebaked = flip M.fromSet uniqueGroups $ \ds ->
      flip foldMap (IS.toList ds) $ \pIx ->
        let pNeighbs = syms pIx
         in T2
              (MIM.MonoidalIntMap $ IM.insertWith (<>) pIx LiveAlone pNeighbs)
  ...
  where
    process mp = (2 ^ numNonZeroes) * perms
      where
        ...
        perms =
          integerFactorial (fromIntegral n)
...
vecRunNeighbs ::
  ...
  [(Int, NCount)]
vecRunNeighbs n mx =
  (\(x :| xs) -> go (mx, n) 0 x True NOne 0 x xs)
  ...
  where
    ...
    go ::
      (Int, Int) ->
      -- \^ running pascal triangle index
      Int ->
      -- \^ running total
      Int ->
      -- \^ original item in that position
      Bool ->
...
vecRunNeighbsInt ::
  ...
  Int ->
  [(Int, Integer)]
vecRunNeighbsInt n mx =
  (\(x :| xs) -> go (mx, n) 0 x True 1 0 x xs)
    . genVecRunIxPascal n mx
  where
    -- we build these in reverse because we can both generate and encode
    -- pascal indices in reverse order in constant space/a streaming way
