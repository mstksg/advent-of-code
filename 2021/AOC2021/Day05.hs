...
import AOC.Common (listV2, traverseLines)
...
data Line = Line
  { lType :: LineType
  , lAxis :: Integer
  -- ^ y for horiz, x for vert, x+y for up, x-y for down
...
overlaps = \case
  Line Horiz y1 rx1 -> \case
    Line Horiz y2 rx2
      | y1 == y2
      , Just (a, b) <- intervalBounds (I.intersection rx1 rx2) ->
          (`V2` y1) <$> [a .. b]
    Line Vert x2 ry2
  ...
  Line Vert x1 ry1 -> \case
    ...
    Line Vert x2 ry2
      | x1 == x2
      , Just (a, b) <- intervalBounds (I.intersection ry1 ry2) ->
          V2 x1 <$> [a .. b]
    Line Upwards z2 rt2
  ...
  Line Upwards z1 rt1 -> \case
    ...
    Line Upwards z2 rt2
      | z1 == z2
      , Just (a, b) <- intervalBounds (I.intersection rt1 rt2) ->
          (\t -> V2 (z1 - t) t) <$> [a .. b]
    Line Downwards z2 rt2
