...
robot :: IntCode -> Map (V2 Int) Int -> Map (V2 Int) Int
robot prog panels = go (step prog) panels (V2 0 0) (V2 0 1)
  where
    go HaltEff panels pos face = panels
...
solve1 :: IntCode -> Int
solve1 prog = Map.size (robot prog Map.empty)

solve2 :: IntCode -> IO ()
solve2 prog = draw (robot prog (Map.singleton (V2 0 0) 1))
