...
robot prog panels = go (step prog) panels (V2 0 0) (V2 0 1)
  where
    go HaltF panels pos face = panels
    go (InputF k) panels pos face = go (k (Map.findWithDefault 0 pos panels)) panels pos face
    go (OutputF c (OutputF t k)) panels pos face = go k panels' pos' face'
      where
