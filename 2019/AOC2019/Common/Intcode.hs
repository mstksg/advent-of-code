...
import AOC.Common
import AOC.Util
import AOC2019.Common.Intcode.Memory
...
toNatural' x =
  maybe (throwing _VMErr (VMEBadPos x)) pure
    . toNatural
    $ x
...
step = do
  ...
  o <-
    maybe (throwing _VMErr (VMEBadInstr x)) pure $
      instr x
