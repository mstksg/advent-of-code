...
import Control.Lens (at, ix, non, uses, (.=), (.~))
  ...
  targ <- uses (at (_gTeam g) . non M.empty) $ \cands ->
    listToMaybe
      ...
      , then
          sortWith
        by
          (Down dmg, Down (effPower h n), _gInitiative h)
      ]
  forM targ $ \t -> do
  ...
  where
    ...
    queue =
      ...
      , then
          sortWith
        by
          (Down (effPower g n), _gInitiative g)
      ]
...
parse24 =
  M.union
    <$> ("Immune System:" *> P.space *> teamParser TImm <* P.space)
