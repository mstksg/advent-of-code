...
import AOC.Solver ((:~>) (..))
import Control.Lens (ifoldMapOf, makeLenses, (-~), (.~))
import Control.Monad (guard)
import Data.Coerce (coerce)
import Data.Fix (Fix)
import Data.Function ((&))
import Data.Functor.Foldable (ana, cata, hylo)
...
stepEntity w es sp e
  ...
  where
    candidates =
      M.fromSet
        ( maybe (error "paintBucket") length
...
data BattleLog a
  = BLTurn (Maybe EType) a -- whether or not there was a kill
  | BLRound a
